...
class Transaction:
    ...
    def send_ipfs_cid(self, recipient: str, ipfs_cid: str) -> None:
        cid_trans_info = {
            "body": {
                "messages": [
                    {
                        "@type": "/test.test.MsgSendipfsCID",
                        "creator": privkey_to_address(self._privkey, self._hrp),
                        "tomodachi": recipient,
                        "ipfsCID": ipfs_cid
                    }
                ],
        ...
        self._msgs.append(cid_trans_info)
...
def encode_signed_tx(signed_tx_file):
    with open(signed_tx_file, "r") as f:
        signed_tx = json.load(f)
    # Encode the JSON object to bytes
    tx_bytes = json.dumps(signed_tx).encode()
    # Base64 encode the bytes
    tx_bytes = base64.b64encode(tx_bytes)
    return tx_bytes

tx_bytes = encode_signed_tx("signed.json")
with open("tx_bytes", "wb") as f:
    f.write(tx_bytes)
